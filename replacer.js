...
function replacePlaceholder(node) {
  if (
    node.placeholder &&
    (node.placeholder.match(TWITTER_REGEXP) ||
      node.placeholder.match(TWEET_REGEXP) ||
      node.placeholder.match(GET_VERIFIED_REGEXP))
  ) {
...
const replaceText = (node) => {
  if (
    node.nodeValue.match(TWITTER_REGEXP) ||
    node.nodeValue.match(TWEET_REGEXP) ||
    node.nodeValue.match(GET_VERIFIED_REGEXP)
  ) {
